...
type Mode = "expense" | "income" | "sale" | "cashout" | "cashin" | "todo" | "note" | null;
const DATA_KEY = "rebuildme-mymoney-v2";
const RECEIVABLES_KEY = "rebuildme-mymoney-receivables-v1";
const SALES_KEY = "rebuildme-mymoney-sales-v1";
...
export default function QuickAdd(){
 ...
 function save(event:FormEvent){
  ...
  if(mode==="expense"||mode==="income"){
   ...
   if(!name.trim()||value<=0)return; const items=readArray(SALES_KEY); localStorage.setItem(SALES_KEY,JSON.stringify([{id:id(),name:name.trim(),amount:value,purchaseDate:date,payments:[]},...items]));
 ...
 {(mode==="sale"||mode==="cashout"||mode==="cashin")&&<><label>Amount<div className="quick-money"><input inputMode="decimal" value={amount} onChange={e=>setAmount(e.target.value.replace(/[^0-9.,\s-]/g,""))} placeholder="0,00"/><span>€</span></div></label>{mode==="sale"&&<div className="quick-amounts">{[50,60,70,80,100,120,130,150].map(v=><button type="button" key={v} onClick={()=>setAmount(String(v))}>{v} €</button>)}</div>}</>}
 {(mode==="sale"||mode==="cashout"||mode==="cashin"||mode==="todo")&&<label>{mode==="sale"?"Purchase date":"Date"}<input type="date" value={date} onChange={e=>setDate(e.target.value)}/></label>}
